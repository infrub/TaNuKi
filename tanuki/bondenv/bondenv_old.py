...
class UnbridgeBondEnv:
    ...
    def optimal_truncate(self, sigma0, chi=20, maxiter=1000, conv_atol=1e-10, conv_rtol=1e-10, conv_sqdiff=-float("inf"), memo=None, algname="NOR", linalg_algname="solve", **kwargs):
        ...
        if algname == "exact_solving":
            ...
            HTA,HTAw,_ = tnd.tensor_eigh(ETA, self.ket_left_labels+self.ket_right_labels, chi=chi*b, decomp_atol=0, decomp_rtol=0, eigh_labels=extraction_label) #TODO sometimes segmentation fault occurs (why?)
            #print(HTAw)
            Mshape = M.shape
            B = N * HTA
