...
class TestBridgeBondOptimalTruncator(unittest.TestCase):
    def test_fullrank(self):
        ...
        ENV = bondenv.BridgeBondOptimalTruncator(V_L, V_R, ["kl"], ["kr"], sigma0)

        Ms,Ss,Ns,er1s,er2s = [],[],[],[],[]
        def wa(chi):
            ENV.set_params(chi=chi)
            ENV.initialize()
            M,S,N = ENV.run()
...
class TestUnbridgeBondOptimalTruncator(unittest.TestCase):
    def test_iroiro(self, b=None, n=None, mc=None): # mc is yosou sareru hitstop of chi
        ...
        for c in range(1,mc+3):
            ENV = bondenv.UnbridgeBondOptimalTruncator(V, ["kl"], ["kr"], sigma0)
            ENV.set_params(chi=c)
            ENV.initialize()
            M,S,N = ENV.run()
            er1 = ((M*S*N)-sigma0).norm()
