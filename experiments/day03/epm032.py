...
def epm0322():
    ...
    registereds = []
    def register(algname, linalg_algname, kwargs, color):
        if len(kwargs)==0:
            title = f"{algname}_{linalg_algname}"
        else:
            title = f"{algname}_{linalg_algname}("
            for kwarg in kwargs.values():
                if type(kwarg) in [float,np.float64]:
                    ...
                    title += f"{kwarg},"
            title = title[:-1] + ")"
        registereds.append( (algname,linalg_algname,kwargs,color,title) )
    ...
    for algname in ["NOR","COR","ROR","IROR","LBOR"]:
        for (linalg_algname,kwargs) in [("cg",{"linalg_maxiter":b}),("solve",{})]:
            register(algname, linalg_algname, kwargs, cm.Paired(i/12))
            i += 1

    metaf = open("epm0322_oups/epm0322_meta.csv","a")
    metaf.write("b,chi,seed,trueError,")
    for (_,_,_,_,title) in registereds:
        metaf.write(title+",")
    metaf.write("\n")
    metaf.close()
    ...
    def ikuze(b,chi,seed):
        ...
        for (algname, linalg_algname, kwargs, color, title) in registereds:
            np.random.seed(seed=seed)
            random.seed(seed)
            print(title, end=": ")
            try:
                memo = {}
                M,S,N = ENV.optimal_truncate(A, maxiter=max(300,b*50), chi=chi, memo=memo, algname=algname, linalg_algname=linalg_algname, conv_atol=-1, conv_rtol=-1, conv_sqdiff=trueError*(1+1e-10), **kwargs)
        ...
        ax2.set_xlim(0,min(100,ax2.get_xlim()[1]))
        plt.legend()
        plt.suptitle(f"epm0322[b={b},chi={chi},seed={seed}]")
        plt.savefig(f"epm0322_oups/[b={b}, chi={chi}, seed={seed}].png", dpi=400)
        #plt.show()
